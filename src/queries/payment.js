...
const addCategoryPaymentAndUpdateInstallments = (CatsMoney, StudentId) => {
    return db.sequelize.transaction(async (t) => {
        ...
        let proms = [];
        CatsMoney.forEach(cat => {
            proms.push(db["PaymentCategory"].create({
        ...
        const firstInstallment = await installment.getCurrentFirstInstallment(StudentId);
        const secondInstallment = await installment.getCurrentSecondInstallment(StudentId);
        // we have 2 seario
        if (totalAmount === (firstInstallment.InstallmentAmount - firstInstallment.InstallmentPaidAmount)) {
        ...
        } else if (totalAmount < (firstInstallment.InstallmentAmount - firstInstallment.InstallmentPaidAmount)) {
            let newAmount = firstInstallment.InstallmentPaidAmount + totalAmount;
            proms.push(db["Installment"].update({
                InstallmentPaidAmount: newAmount,
            }, {
            ...
            if ((firstInstallment.InstallmentAmount - firstInstallment.InstallmentPaidAmount) !== 0) {
                let newAmount = firstInstallment.InstallmentAmount - firstInstallment.InstallmentPaidAmount;
                proms.push(db["Installment"].update({
                    InstallmentPaidAmount: newAmount,
                    Status: 'PAID',
                    InstallmentFullyPaidDate: PaymentDate
            ...
            if(newAmount === (secondInstallment.InstallmentAmount - secondInstallment.InstallmentPaidAmount)){
                proms.push(db["Installment"].update({
                    InstallmentPaidAmount: newAmount,
                    Status: 'PAID',
                    InstallmentFullyPaidDate: PaymentDate
